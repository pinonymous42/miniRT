...
int sphere_t_val(t_vec3 start_vec, t_vec3 dir_vec,t_objects *object_list)
{
	t_vec3 camera2sp_vec = vec3_sub(start_vec, object_list->vec); // カメラから球へのベクトル
	int t1, t2;
...
void get_t_value(t_vec3 start_vec, t_vec3 dir_vec, t_objects *object_list, double *t, int i) // 交差判定
{
	if (object_list[i].kind == sp)
		t[i] = sphere_t_val(start_vec, dir_vec, &object_list[i]);

...
/*   Updated: 2023/04/18 14:55:32 by tasano           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_condition get_condition_cylinder(t_cylinder *cylinder, t_vec3 start_vec, t_vec3 dir_vec)
{
	...
	con.c = vec3_mag(tmp_vec_and_cross) * vec3_mag(tmp_vec_and_cross) - pow(cylinder->diameter, 2);
...
t_condition get_condition_shpere(t_sphere *sphere, t_vec3 start_vec, t_vec3 dir_vec)
{
	...
	con.c = vec3_dot(camera2sphere_vec, camera2sphere_vec) - sphere->diameter * sphere->diameter;
	con.d = con.b * con.b - 4 * con.a * con.c;
	if (con.a == 0 ||con.d == 0 || con.d < 0)
		return (con);
	con.t = get_tval(con);

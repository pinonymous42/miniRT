...
int		main_loop(t_game *game)
{
	...
	t_vec3 plain_normal_vec = vec3_normalize(vec3_init(0, -1, 0));//平面の法線ベクトル
	t_vec3 plain_position_vec = vec3_init(0, 1, 0);//平面の任意の点の位置ベクトル
	...
	for (double y = 0; y < HEIGHT; y++)
	{
		for (double x = 0; x < WIDTH; x++)
		{
			...
			double t;
			if (vec3_dot(dir_vec, plain_normal_vec) == 0)
				t = 0;
			else
				t = (vec3_dot(vec3_sub(plain_position_vec, camera_vec), plain_normal_vec) / vec3_dot(dir_vec, plain_normal_vec));
			if (t > 0)
			{
				//拡散反射
				t_vec3 crosspoint_vec = vec3_add(camera_vec, vec3_mul(dir_vec, t));
				t_vec3 incident_vec = vec3_normalize(vec3_sub(light_vec, crosspoint_vec));
				double dot_1;
				if (vec3_dot(plain_normal_vec, incident_vec) <= 0)
					dot_1 = 0;
				else
					dot_1 = vec3_dot(plain_normal_vec, incident_vec);
				double diffusion = dot_1 * light_power * material.kDif;
				//鏡面反射
				t_vec3 reverse_dir_vec = vec3_normalize(vec3_mul(dir_vec, -1));//視線ベクトルの逆向き
				t_vec3 regular_reflection_vec = vec3_normalize(vec3_sub(vec3_mul(vec3_mul(plain_normal_vec, vec3_dot(incident_vec, plain_normal_vec)), 2), incident_vec));
				double dot_2;
